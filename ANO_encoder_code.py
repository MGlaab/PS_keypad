...
# setup neopixel
pixels = neopixel.NeoPixel(board.NEOPIXEL, 1, brightness=0.008, auto_write=False)

# setup encoder
#encoder = rotaryio.IncrementalEncoder(board.D10, board.D9
encoder = rotaryio.IncrementalEncoder(board.D12, board.D13)
last_position = 0
current_position = 0
position_change = 0
movement = False
...
#set grounds for switches and encoder
ComA = digitalio.DigitalInOut(board.D11)
ComA.direction = digitalio.Direction.OUTPUT
ComB = digitalio.DigitalInOut(board.SDA)
ComB.direction = digitalio.Direction.OUTPUT

# setup mode button
#MODE_BUTTON = digitalio.DigitalInOut(board.D12)
MODE_BUTTON = digitalio.DigitalInOut(board.D10)
MODE_BUTTON.direction = digitalio.Direction.INPUT
MODE_BUTTON.pull = digitalio.Pull.UP

# left arrow
LEFT_ARROW = digitalio.DigitalInOut(board.D5)
LEFT_ARROW.direction = digitalio.Direction.INPUT
LEFT_ARROW.pull = digitalio.Pull.UP

# up arrow
UP_ARROW = digitalio.DigitalInOut(board.D6)
UP_ARROW.direction = digitalio.Direction.INPUT
UP_ARROW.pull = digitalio.Pull.UP

# right arrow
RIGHT_ARROW = digitalio.DigitalInOut(board.D9)
RIGHT_ARROW.direction = digitalio.Direction.INPUT
RIGHT_ARROW.pull = digitalio.Pull.UP

# down arrow
DOWN_ARROW = digitalio.DigitalInOut(board.SCL)
DOWN_ARROW.direction = digitalio.Direction.INPUT
DOWN_ARROW.pull = digitalio.Pull.UP
